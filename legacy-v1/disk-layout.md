...
**For agents:** Start with [LEGACY.md](LEGACY.md) § Agent read order. **Read with the Read tool** when Stages A or C run, when a Stage D job starts, or when resuming.
...
Job folder schema: [job-state.md](../../physics-paper-editing/legacy-v1/job-state.md).
