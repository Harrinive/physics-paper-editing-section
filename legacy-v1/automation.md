...
**For agents:** Start with [LEGACY.md](LEGACY.md) § Agent read order. **Read when** resuming a section edit or arming an optional file watcher. Resume boot: [cross-skill.md](cross-skill.md) § ON RESUME.
...
Silent editor edits are invisible until a chat message or a wave finishes, unless a debounced watcher (~5–10s after last write) wakes the parent and runs the same hash check ([coworker-loop.md](../../physics-paper-editing/legacy-v1/coworker-loop.md)). Arm only if the user wants that; do not mention the watcher in the narrative.
